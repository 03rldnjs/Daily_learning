...
## 💡 배운 점 (Retrospective)
### 왜 `from tkinter import *`보다 `import tkinter as tk`인가?
1. **이름 충돌(Name Collision) 방지:** 내장 함수나 변수가 덮어씌워져 발생하는 원인 모를 버그를 예방합니다.
2. **명확한 출처(Readability):** `tk.` 접두어를 통해 어떤 위젯이 어떤 라이브러리에서 왔는지 한눈에 파악할 수 있어 협업과 유지보수에 유리합니다.
