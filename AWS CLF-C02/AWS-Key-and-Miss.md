...
- 4가지 핵심 기능별 문제 유형 예시
  ...
  - 특정 EBS 볼륨이 삭제되기 전에 이 볼륨이 어떤 EC2 인스턴스에 연결되어 있었는지 연관관계를 확인하고 싶다
...
# AWS 인프라 단위
Region(리전) = 도시
VPC = 도시 전체를 둘러싼 커다란 성벽/울타리(Region 계층)
AZ(가용 영역) = 도시 안에 격리된 구/동
Subnet = 구/동 안에 들어선 특정 아파트 단자(AZ 계층)
-> subnet은 무조건 1개의 AZ에만 귀속된다
-> 단, 1개의 AZ에 여러 subnet 존재는 가능

# AMI(Amazon Machine Image)
- EC2 인스턴스를 찍어내기 위한 미리 완성된 OS 및 소프트웨어 패키지 틀
- OS 뿐만 아니라 필요한 프로그램, 환경 설정, EBS 볼륨 상태(스냅샷)까지 통째로 저장해 둔 템플릿이라서, AMI만 있으면 똑같은 설정의 EC2 인스턴스를 몇 초 만에 수십대씩 즉시 복제해서 띄울 수 있음
- 핵심 특징 4가지
1. 완전한 시스템 백업/템플릿: EC2 생성에 필요한 모든 정보가 포함되어 있음
2. 리전 종속적: AMI는 특정 리전에 생성/저장됨. 다른 리전에서 같은 AMI로 EC2를 띄우려면 별도의 기능을 사용해야 함
3. 재사용성 및 가속화: 인스턴스를 켤 때마다 User Data 스크립트로 하나씩 앱을 설치하는 것보다, Custom AMI로 쉽게 켜는게 부팅 및 서비스 준비 속도가 휠씬 빠름
4. 보안 및 권한 관리: AMI를 개인만 쓸 수도 있고, 다른 AWS 계정과 공유하거나, 전체 공개 또는 AWS Marketplace에 판매할 수도 있음
- AMI가 내결함성을 제공하는 과정
  1. 빠른 복제 및 대체 인스턴스 자동 생성
     - 특정 EC2 인스턴스가 물리적 장애로 다운되었을 때, 미리 설정해 둔 AMI가 있다면 수초~수분 만에 동일한 환경의 새 인스턴스를 즉시 띄워 대체 가능
  2. Auto Scaling 및 멀티 AZ 복구와의 결합
     - Auto Scaling 그룹은 특정 가용 영역의 인스턴스가 고장나면, AMI를 사용해 다른 정상 AZ에 동일한 인스턴스를 즉시 자동으로 복제/생성함
  3. 지역 간 복제(Cross-Region Disaster Recovery)
     - AMI를 다른 리전으로 복제할 수도 있음. 특정 리전 전체에 재난이나 대규모 장애가 발생해도, 다른 리전에서 해당 AMI로 즉시 동일한 시스템을 재구성하여 서비스를 계속 유지 가능
    
# AWS EBS(Elastic Block Storage) 암호화 대상
-> 유형을 가리지 않고 100% 다 암호화 가능
- 루트 볼륨, 데이터 볼륨, EBS 스냅샷 등 어떤 유형이든 사용자가 원하면 암호화 적용 가능
- 단, S3처럼 자동 암호화가 기본값으로 제공되지는 않음
- 매번 암호화를 활성화하는 것이 번거롭다면, EBS 기본 암호화를 활성화하여 자동화할 수 있음(해당 설정을 활성화하면 S3처럼 새로 만드는 모든 EBS 볼륨이 자동으로 암호화됨)
...
1. S3(객체 스토리지)
   - 요금 발생 요인:
     - 저장 용량
     - 요청 횟수
     - 데이터 아웃바운드 전송량(Data Egress)
   - 요금 미발생: 데이터 업로드(Data Ingress/inbound data transfer), 동일 리전 내 다른 AWS 서비스로의 전송(outbound라도 해당 경우에는 요금 미발생)
2. EC2 (컴퓨팅)
   - 요금 발생 요인
     - 인스턴스 실행 시간(초 단위)
     - 인스턴스 타입 및 사양(vCPU, Memory)
     - OS 라이선스(Windows, RedHat 등)
     - 연결된 EBS 볼륨 용량 및 데이터 아웃바운드 전송량
3. EBS(블록 스토리지)
   - 요금 발생 요인:
     - 할당된 용량: EC2를 꺼두더라도 EBS를 할당받아 둔 용량 전체에 대한 배용 발생(실제 사용량 기준이 아닌 할당량 기준)
     - 성능 스펙: 고성능 옵션 추가 시 비용 증가(하드디스크의 성능 ex.빠른 I/O 등)
     - 스냅샷 저장 용량(S3에 저장되는 실제 사용량)
4. Lambda(서버리스 컴퓨팅)
   - 요금 발생 요인:
     - 호출 횟수(Request count)
     - 실행 시간 및 메모리 할당량(GB-seconds)
   - 특징: 코드가 실행되지 않고 대기 중일 때는 요금 발생 X
5. DynamoDB(NoSQL DB)
   - 요금 발생 요인:
     - 온디맨드 방식: 읽기/쓰기 요청 단위당 과금
     - 프로비저닝 방식: 미리 설정해 둔 읽기/쓰기 용량 단위
     - 데이터 저장 용량(Per GB/month)
    
# AWS Lambda의 핵심 특징
- 이벤트 기반 완전 관리형 서버리스 컴퓨팅 서비스
- 동동 실행 모델(Concurrent Executions)
  - Lambda에 요청이 들어오면 AWS는 해당 코드를 실행할 격리된 환경(컨테이너)를 즉시 할당함.
  - 요청의 개수에 맞는 환경이 자동으로 동시에 병렬 생성(Scale-out)됨
- 지속적/연속적 스케일링(Continuous Scaling)
  - 사용자가 EC2처럼 Auto Scaling 그룹을 만들거나 CPU 사용률 임계값을 설정할 필요 없이 트래픽이 들어오는 수량에 비례해 밀리초 단위로 수평 확장이 연속적/지속적으로 작동하고, 요청이 끝나면 알아서 소멸됨
 
# Internet Gateway & NAT Gateway
-> Public Subnet에 양방향 인터넷을 열어줌 = Internet Gateway
-> Private Subent에서 밖으로 나가는 인터넷만 열어주는 건 NAT Gateway

# Amazon Lightsail
- 초보자 및 소규모 프로젝트를 위한 올인원 가상 전용 서버 서비스
- 특징:
  - 복잡함 없는 올인원 패키지: EC2, VPC, EBS, Security Group 등을 일일이 세부 설정해야하는 기존 AWS 서비스와 달리, 컴퓨팅, 메모리, 스토리지, 데이터 전송 대역폭을 하나로 묶어 클릭 한 번으로 간편하게 시작 가능
  - 예측 가능한 고정 월정액 요금: 사용량에 따른 변동 요금이 아닌, 예측 가능한 고정 가격 플랜 제공
  - 주요 용도: 블로그(WordPress), 개인 포트폴리오 사이트, 소규모 웹 앱, 테스트 및 개발 환경 구축
  + 서버 외에도 완전 관리형 관계형 데이터베이스인 MySQL 및 PostgreSQL을 클릭 몇 번으로 간단히 생성할 수 있는 독립형 DB 제공
 
# AWS Resource Access Manager (AWS RAM)
- 여러 AWS 계정 간에 특정 인프라 리소스를 중복으로 생성하지 않고 안전하게 공유하여 사용할 수 있게 해주는 완전 관리형 서비스
- AWS RAM이 해결하는 핵심 문제(서비스 사용 이유)
  - 중복 생성 방지 및 비용 절감:
    - 계정마다 동일한 리소스를 개별 구축하면 관리 포인트가 늘어나고 라우팅 비용이 늘어남. AWS RAM을 사용하면 중앙 네트워크/보안 계정 1곳에 리소스를 만들어 두고 타 계정에 공유해 공유 자원으로 활용할 수 있음
  - 운영 오버헤드 감소:
    - 중앙에서 하나의 리소스만 업데이트/패치/관리하면 되므로 관리 효율성이 크게 향상됨
  - 세분화된 권한 제어:
    - 공유받은 계정이 해당 리소스를 단순히 읽기/사용만 할 수 있는지, 일부 수정까지 가능한지 등의 접근 권한 정책을 상세히 부여할 수 있음
- AWS RAM으로 공유 가능한 주요 리소스
  - 네트워크
  - 컴퓨팅
  - 보안 & 라이선스
  - 데이터 등
...
# AWS CAF 클라우드 도입 4단계
1. Envision (구상 단계)
   - 역할: 클라우드가 자사의 비즈니스에 줄 수 있는 기회를 식별하고 목표를 설정하는 단계
   - 주요 키워드: 비즈니스 기회 식별, 목표 설정, 디지털 혁신 비전 수립
2. Align (정렬/조정 단계)
   - 역할: 6가지 관점 전체에 대해 역량 갭을 식별하고, 조직의 준비 상태를 정렬하는 단계
   - 주요 키워드: Identify capability gaps, Align readiness, 역량 부족 파악, 조직 정렬 및 로드맵 수립
3. Launch (출시/파일럿 단계)
   - 역할: 실무에서 클라우드 가치를 증명하기 위해 초기 파일럿 프로젝트를 구축하고 실행하는 단계
   - 주요 키워드: Pilot projects, MVP(최소 기능 제품) 구현, 가치 증명
4. Scale (확장/전면 도입 단계)
   - 역할: 성공한 파일럿 경험을 바탕으로 전사적인 수준으로 클라우드 적용 범위를 확장하고 비즈니스 가치를 극대화하는 단계
   - 주요 키워드: Expand, Production environment, 지속적 개선(Continuous improvement), 전사적 확장

# AWS CAF 6가지 관점 (6 Perspectives)

[비즈니스 중심]
1. Business (비즈니스): "비즈니스 목표, ROI, 전략적 가치"
2. People (인력): "문화 변화, 직원 교육/역량, 인사 조직"
3. Governance (거버넌스): "리스크 관리, 비용 관리(FinOps), 프로그램 통제"
